...
1. Extração (Extract)
Pandas: 
Importar e organizar dados da planilha (CSV/Excel).
2. Transformação (Transform)
(Pandas & NumPy):
Limpeza e preparação dos dados por relevância (influência dos pais, atividades extracurriculares, tempo de estudo).
Criação de novas variáveis derivadas (média de horas de estudo x semana).
Cálculos matemáticos e estatísticos de correlação entre variáveis (estudo x GPA).
Normalização de valores numéricos para padronizar escalas.
